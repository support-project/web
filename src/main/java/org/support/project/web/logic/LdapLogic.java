...
public class LdapLogic {
	 ...
	 * Ldapログイン処理
	 * 
	 * はじめに、検索用のユーザでLdapに接続し、ログイン用のユーザのDNを検索する
	 * その後に、ログインを実行する。
	 * こうすることでldapLogin1で発生したBASE DNが固定という問題を解消できる
	 * 
	 * 
	 * @param entity
	 * @param id
	...
	private LdapInfo ldapLogin2(LdapConfigsEntity entity, String id, String password) throws LdapException, IOException {
		...
		try {
			...
			return info;
		} catch (LdapException | CursorException | InvalidKeyException | NoSuchAlgorithmException | NoSuchPaddingException | IllegalBlockSizeException | BadPaddingException e) {
			//認証失敗
			return null;
		} finally {
			if (cursor != null) {
	 ...
	 * はじめに作成したLdapLogin処理
	 * 
	 * BASE DNが固定になってしまい、サブツリー上のユーザでログイン出来ない問題がある
	 * Knowledge 0.5.3 でリリースしてしまったので、それをそのまま使えるように残しておくが、
	 * 上記のldapLogin2を使うように変更を促していく
	 * 
	 * @deprecated 
	 * @param entity
	 * @param id

...
public class HttpUtil {
	 ...
	 * (request.getParameter("item").getBytes("ISO-8859-1"));
	 ...
	 * 
	 * <Connector port="8080" <!-- * * なんかいっぱい書いてある * -->
	 * useBodyEncodingForURI="true" /> って
	 * 
	 * useBodyEncodingForURI="true" を追記してあげるといい
	 * 
	 * もしくは
	 * 
	 * <Connector port="8080" <!-- * * なんかいっぱい書いてある * --> URIEncoding="UTF-8" />
	 * という感じで
	 * 
	 ...
	 * @author Koda
	 */
	public static void forward(HttpServletResponse res, HttpServletRequest req,
	...
	public static void redirect(HttpServletResponse res,
			HttpServletRequest req, String page) throws IOException,
			ServletException {
	 ...
	 */
	public static <T> T parseRequest(HttpServletRequest req,
			Class<? extends T> paramtypes) throws InstantiationException,
			IllegalAccessException, JSONException, IOException, InvalidParamException {
	 ...
	 */
	public static <T> T parseRequest(HttpServletRequest req,
			Class<? extends T> paramtypes, String prefix)
...
//					Object value = ValueConverter.conv(values, type);
	...
	public static <T> T getParameter(HttpServletRequest req, String param, Class<? extends T> paramtypes) throws InvalidParamException {
		...
		if (values == null || values.length == 0) {
			...
			if (paramtypes.isAssignableFrom(value.getClass())) {
				...
				log.error("fail getParameter   [param]" + param + "   [type]" + paramtypes.getName()
						+ "   [value]" + PropertyUtil.reflectionToString(value));
	...
	public static LoginedUser getLoginedUser(HttpServletRequest request) {
		return (LoginedUser) request.getSession().getAttribute(
	...
	public static boolean isSmartphone(HttpServletRequest request) {
		String sUserAgent = request.getHeader("user-agent");
		if ((sUserAgent.indexOf("iPhone") > 0 && sUserAgent.indexOf("iPad") == -1)
				|| sUserAgent.indexOf("iPod") > 0
